...
1. 把素材放进 `materials/`；
2. 理解素材：`curl -X POST http://localhost:61801/scan -H "Content-Type: application/json" -d '{"folder":"."}'`（或等空闲调度）；
3. 剪辑：浏览器打开 `http://localhost:61803`。
...
- **全本地**（隐私最好）：`PLANNER_BASE_URL=http://localhost:61800/v1`、`PLANNER_MODEL=qwen2.5vl:3b`、API Key 留空。
...
- ✅ 网页工作台（两个 Tab，对应后台服务）：**① 素材库·提取字幕**（=分析服务 :61801：整目录批量提取台词字幕；按时间规则批量剪也在这页）→ **② 字幕剪辑·手动精剪**（=:61803+:61802：提取台词→勾选句子或一句话智能勾选→剪出片段→下载成片/烧硬字幕/导出剪映草稿/SRT；时间线可拖边裁剪/排序/分割/快捷键）
