...
| 组件 | 作用 |
| ---- | ---- |
| ai-video-studio/services/analyzer | L1 理解：场景切分/ASR/VLM 总结/向量（端口 57801） |
| ai-video-studio/services/planner | L2 方案：对话/剧本/场记单 API + 前端（57803） |
| ai-video-studio/services/executor | L3 执行：ffmpeg 剪辑（57802） |
...
- 三服务 + Ollama：analyzer 57801 / executor 57802 / planner(Web) 57803 / Ollama 57800；原生模式 start_local.bat（无 Docker，推荐）
...
- 前台已重构为三 Tab 对应后台服务：①素材分析=analyzer:57801（含免分析批量剪）②剪辑出片=planner:57803+executor:57802（素材勾选→AI自动剪/手动精剪/场记单→成片/剪映草稿/SRT）③进阶剧本=planner:57803；原目录页"时间线剪辑"下拉入口与步骤条已删（loadEditFiles/openEditFromSel/markStep 已移除），手动精剪统一走②勾选入口，showTab() 切换
